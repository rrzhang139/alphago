#!/usr/bin/env python3
"""Go 9x9 warm-start with low LR — preserve learned knowledge.

Hypothesis: cosine LR 0.002 is too aggressive for warm-start, causing
catastrophic forgetting. LR 0.0005 with cosine→1e-5 should preserve
the warm-start model's knowledge while still improving.
...
WARM_START = os.path.join(EXPERIMENT_DIR, '..', '20260312_go9_gnugo', 'data', 'checkpoints', 'warm_start.pt')
...
def main():
    ...
    config = AlphaZeroConfig(
        ...
        training=TrainingConfig(
            lr=0.0005,              # 4x lower than default
            weight_decay=1e-4,
            batch_size=256,
    ...
    if os.path.exists(WARM_START):
        ...
        print(f"WARNING: No warm-start at {WARM_START}")

    print("Training with LOW LR (0.0005, cosine→1e-5)")
