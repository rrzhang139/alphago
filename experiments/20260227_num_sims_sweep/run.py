#!/usr/bin/env python3
"""Experiment: How much does MCTS search depth matter?

Sweep num_simulations = [1, 5, 10, 25, 50, 100] with everything else at defaults.
Measures: vs_random win rate, loss curves, policy entropy, training time.

Hypothesis: More simulations = stronger play, but with diminishing returns.
At 1 sim, MCTS adds almost nothing. At 100, it's overkill for tic-tac-toe.
The interesting question: where's the knee?
"""
...
# Fixed training params — keep it short enough to run all 6 in reasonable time
...
def plot_comparison(all_results: dict):
    ...
    width = 0.35
    bars1 = ax.bar(x - width/2, final_wr, width, label='Final vs Random %',
                   color='#2ecc71', alpha=0.7)
    ax.set_ylabel('Win Rate %', color='#2ecc71')
    ax.set_ylim(0, 110)

    ax2 = ax.twinx()
    bars2 = ax2.bar(x + width/2, wall_times, width, label='Wall Time (s)',
    ...
    fig.suptitle('num_simulations Sweep — Tic-Tac-Toe', fontsize=13, fontweight='bold')
...
def print_summary(all_results: dict):
    ...
    for sims in sorted(all_results.keys()):
        ...
        print(f"{sims:>6}  {final_wr:>11.0%}  {best_wr:>11.0%}  {final_loss:>10.3f}  "
              f"{final_entropy:>11.2f}  {wall:>7.1f}s")
